...
void parse_rule_string(std::unordered_map<std::string, std::vector<std::vector<std::string>>> &rules,
                       const std::string &rule_str) {
  // Separate the rule's name from its definition which contains a list of rule sequences to match
  int name_delimiter = rule_str.find(std::string(": "));
  std::string rule_name = rule_str.substr(0, name_delimiter);
  ...
  do {
    match_end = rule_str.find(std::string(" | "), match_start);
    std::stringstream rule_seq_ss = std::stringstream(rule_str.substr(match_start, match_end - match_start));
...
int main() {
  ...
  while(std::getline(file, line)) {
    ...
    // Update rules 8 and 11 if needed
    if(line.substr(0, 3) == std::string("8: ")) {
      parse_rule_string(rules, "8: 42 | 42 8");
    } else if(line.substr(0, 4) == std::string("11: ")) {
      parse_rule_string(rules, "11: 42 31 | 42 11 31");
    } else {
      parse_rule_string(rules, line);
    }
  }
