...
int main() {
  ...
  // For each bag, create a list of parent bags that contain it
  std::unordered_map<std::string, std::vector<std::string>> bag_parents;
  for(const std::pair<std::string, std::unordered_map<std::string, int>> &bag : bags) {
    bag_parents[bag.first] = std::vector<std::string>();
  }

  for(const std::pair<std::string, std::unordered_map<std::string, int>> &bag : bags) {
    for(const std::pair<std::string, int> &contained_bag : bag.second) {
      bag_parents[contained_bag.first].push_back(bag.first);
    }
  }

  // Initialize DFS helper variables
  std::vector<std::string> dfs_stack;
  dfs_stack.push_back(std::string("shiny gold"));
  ...
  // Use DFS to find all bags that eventually contain the shiny gold bag
  while(dfs_stack.size() > 0) {
    std::string elem = dfs_stack.back();
    dfs_stack.pop_back();
    if(!visited[elem]) {
      visited[elem] = true;
      for(std::string &parent : bag_parents[elem]) {
        valid_set.insert(parent);
  ...
  // Print result
  std::cout << valid_set.size() << std::endl;
  return 0;
}
